...
const BOOKMARKS_COLLECTION = 'user_bookmarks';
...
/**
 * Check if current user (authenticated only) has bookmarked a prompt
 */
export async function getUserBookmarkStatus(promptId) {
  const user = auth.currentUser;
  
  // Only authenticated users can have bookmarks
  if (!user) {
    return { bookmarked: false };
  }
  
  try {
    const bookmarkDocId = `${user.uid}_${promptId}`;
    const bookmarkRef = doc(db, BOOKMARKS_COLLECTION, bookmarkDocId);
    const docSnapshot = await getDoc(bookmarkRef);
    
    return { bookmarked: docSnapshot.exists() };
  } catch (error) {
 ...
 */
export async function addUserBookmark(promptId) {
  const user = auth.currentUser;
  
  if (!user) {
    return {
      success: false,
      message: 'Must be logged in to bookmark'
    };
  }
  ...
  try {
    ...
    console.log(`Bookmark added for prompt: ${promptId}`);
    return {
      success: true,
 ...
 */
export async function removeUserBookmark(promptId) {
  const user = auth.currentUser;
  ...
  try {
    ...
    console.log(`Bookmark removed for prompt: ${promptId}`);
    return {
      success: true,
 ...
 */
export async function toggleUserBookmark(promptId) {
  const user = auth.currentUser;
 ...
 * Calls callback with array of bookmarked prompt IDs
 */
export function listenToUserBookmarks(callback) {
  const user = auth.currentUser;
  ...
  try {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const bookmarkedIds = snapshot.docs.map(doc => doc.data().promptId);
